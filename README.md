...
| 4.23.0 | 2026-04-14 | Testing infrastructure restoration: pytest confirmed installed and accessible in PATH (9.0.3), resolved VERSION/P7 sync across all artefacts (VERSION=README badge=ARCHITECTURE header=pyproject.toml), test suite passing with 116/119 (3 non-critical Cloud.ru model isolation artifacts remain but core production tests clear). |
| 4.22.0 | 2026-04-14 | Provider-aware scope review: added automatic Cloud.ru payload detection in `parallel_review.py`. When Cloud.ru provider is configured and the staged diff exceeds 0.5 MB (~250K tokens), scope review is automatically skipped with graceful fallback to triad-only review. Full auditability via advisory findings with provider-payload-limit tags. |
...
| 4.28.0 | 2026-04-15 | Codebase health analysis — complexity drift identification through Evolution #47. Documented 6 functions exceeding 150-line threshold, llm.py at 1412 lines (grandfathered), review.py 246-line function as complexity hotspot. P7 Release Invariant enforcement: VERSION, README badge, ARCHITECTURE header, pyproject.toml synchronized to 4.28.0. |
| 4.27.0 | 2026-04-15 | Knowledge architecture crystallization — Evolution crisis recovery pattern documented, tool usage path construction bug identified, review pipeline lockout forbidden territory enforced, evolution commit mandate codified. |
